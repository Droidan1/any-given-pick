...
The committed score workflow requests a sync every ten minutes during typical Thursday-through-Monday game windows, checks upcoming reference lines three times daily, and can also be run manually. Opening an unlocked picks page requests a non-blocking check when stored odds are missing or older than two hours. The hourly **Send player notifications** GitHub workflow calls `/api/cron/emails` for email, Web Push, and native iPhone reminders and retries. It must be present on the repository's default branch (`main`) for scheduled runs to activate. Publishing and submitting trigger immediate delivery attempts; score updates and manual final-score/cancellation changes check for completed-week results notifications after the response, without delaying score reads. Results require every game to be final or canceled and at least one final game. Two once-daily Vercel Crons in `vercel.json` independently backstop score and notification processing if GitHub scheduling is interrupted. Both protected routes use the same `CRON_SECRET`, and the notification workflow reuses the existing `SCORE_SYNC_CRON_SECRET` repository secret. The health watchdog treats a sync older than 40 minutes as stale during live windows and 30 hours outside them. This bounded schedule stays within the intended beta operations budget while deterministic delivery keys keep overlapping runs safe. Scheduled jobs can be delayed, so the Admin settings health panel records the latest score attempt, success, provider warning, and update count. Commissioners can always enter a final score manually under **Admin settings → Manage contest weeks**.

To verify the notification scheduler safely, manually run **Send player notifications** with `dry_run=true` (the default). It checks production authorization, delivery-table access, and channel configuration through `/api/cron/emails?dryRun=true`; it never queues or sends notifications and does not contact Apple/Resend/Clerk. This is a readiness check, not a delivery receipt. Scheduled runs use `dryRun=false`. Do not publish test weeks or submit synthetic player cards on production to test event hooks.
