    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        ...
        if (input.intent === "delete") {
          try { window.localStorage.removeItem(userDraftStorageKey(draftOwnerId, week.id, result.boardId, form.board?.resetRevision)); } catch { /* Storage is optional. */ }
        }
        router.refresh();
  ...
  return <section className="player-boards" aria-labelledby="your-boards-title">
    ...
    <div className="board-list-content"><header className="board-list-heading"><h2>Your boards</h2><span>{active.length} / {limit} used</span></header>
      ...
      {active.map(board => <article className="player-board-row" key={board.id}>
        ...
        <div className="player-board-identity"><h3>{board.boardName}</h3><p>{board.status === "disqualified" ? "Disqualified" : board.currentVersionNumber > 0 ? "✓ Submitted" : `${Object.keys(board.draftPicks).length} of ${week.games.length} picks · Draft`} {board.lastResetAt && board.currentVersionNumber === 0 ? " · Reset by admin" : ""} · {week.seasonPhase === "preseason" ? "Tiebreaker" : "Monday"} {board.mondayPrediction ?? "—"}</p></div>
        ...
        {board.status !== "disqualified" && <div className="board-actions player-board-tools"><button type="button" disabled={!editable} onClick={() => openForm("rename", board)}>Rename</button>{week.boardSettings.multipleBoardsEnabled && <button type="button" disabled={!canCreate} onClick={() => openForm("create", board)}>Copy board</button>}{board.boardNumber !== 1 && board.currentVersionNumber === 0 && !board.lastResetAt && <button type="button" disabled={!editable} onClick={() => openForm("delete", board)}>Delete draft</button>}</div>}
      </article>)}
